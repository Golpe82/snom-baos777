        ...
        for row in reader:
            if row["Group name"] == "Group name":
                ...
                Groupaddress.objects.create(
                    ...
                    html_action=_get_dom_action(
                        datapoint_type, groupaddress, groupaddress_name
                    ),
                )
...
def _get_dom_action(datapoint_type, groupaddress, groupaddress_name):
    ...
    if datapoint_type == "DPST-1-11":
        ...
        return "<form method='get'>" f"{on}" f"{off}" "</form>"
        ...
        return "<form method='get'>" f"{plus}" f"{minus}" "</form>"
