...
class HTTPHandler:
    ...
    def handle(self, exception=None):
        if self.response.status_code == HTTPStatus.UNAUTHORIZED:
            raise Exception(f"Unauthorized. Credentials: {self.credentials}") from exception
    ...
    def retry_after_login_block(self):
        ...
        logging.error(f"Too many requests. Login blocked for {seconds_until_login_unblocked} seconds")
